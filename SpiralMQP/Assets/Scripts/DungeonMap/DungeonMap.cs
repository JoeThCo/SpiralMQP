...
public class DungeonMap : SingletonAbstract<DungeonMap>
{
    ...
    [SerializeField] private GameObject minimapUI; // We will need to deactivate the minimap when the overview map is activated, so we need a reference of the minimap UI gameobject
    ...
    private void Update()
    {
        // If left mouse button pressed and gamestate is dungeon overview map then get the room clicked
    ...
    private IEnumerator MovePlayerToRoom(Vector3 worldPosition, Room room)
    {
        ...
        SoundEffectManager.Instance.PlaySoundEffect(teleportSoundEffect);
        ...
        GameManager.Instance.GetPlayer().playerControl.EnablePlayer(false);
    ...
    /// <summary>
    /// Clear the dungeon overview map UI and return to the main camera
    /// </summary>
